...
const PAGE = getExperiencePage('healing-retreat-himalayas')!;
const PATH = '/healing-retreat-himalayas';

const FAQ_ITEMS = [
  {
    question: 'What is a healing retreat in the Himalayas?',
    answer:
      'A healing retreat is a structured program designed to support emotional, physical, and psychological recovery through nature immersion, somatic practices, meditation, and rest. Unlike a holiday or spa, a healing retreat creates conditions for the body and mind to process what they have been holding.',
  },
  {
    question: 'Do I need to talk about my healing journey?',
    answer:
      'No. Our healing retreats are designed so that the healing happens through environment, rest, and somatic release — not through talking therapy. You are welcome to share, but there is no requirement. Many participants find that the processing happens naturally through the body and the landscape.',
  },
  {
    question: 'How long should a healing retreat be?',
    answer:
      'Three days is a meaningful reset for gentle healing. Five to seven days allows for deeper processing. Ten days or more is where the most profound shifts occur. The right duration depends on what you are carrying and what your schedule allows. Many participants start with 5 days and return for longer when they see the impact.',
  },
  {
    question: 'Can I combine a healing retreat with other programs?',
    answer:
      'Yes. Healing retreats can be combined with meditation, silent retreat, or gentle trekking formats depending on your energy level. Some participants add trekking for physical release. Others add silence for deeper rest. Reach out and we will help you design the right combination.',
  },
];
...
export default function HealingRetreatHimalayasPage() {
  ...
  const serviceSchema = generateServiceSchema(
    { title: 'Healing Retreat Himalayas', description: 'Deep rest and emotional healing in the Himalayan mountains.' },
  ...
  return (
    <TrackedPage page={PATH} style={{ maxWidth: '100%', margin: '0 auto', padding: 0, overflowX: 'hidden' }}>
      <AutoArticleSchema
        title="Healing Retreat Himalayas"
        description="Deep rest and emotional healing in the Himalayan mountains."
        path={PATH}
      />
      ...
      <style>{`
        ...
        .med-healing-cta { padding: 3rem 0; text-align: center; }
        .med-healing-cta .med-body { max-width: 42rem; margin: 0 auto 1.5rem; }
        ...
        .med-deep-item .med-section-number { display: inline-flex; align-items: center; justify-content: center; width: 44px; height: 44px; border-radius: 50%; background: rgba(15,118,110,0.08); color: #0f766e; font-family: var(--font-inter), sans-serif; font-size: 0.8rem; font-weight: 700; margin-bottom: 1rem; }
        .med-deep-item .med-featured-badge { position: absolute; top: 1rem; left: 1rem; background: #0f766e; color: #fff; padding: 0.3rem 0.8rem; border-radius: 999px; font-size: 0.6rem; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; }
      `}</style>
        ...
        <div className="med-outer">
          <Breadcrumb
            items={[
              ...
              { name: 'Healing Retreat Himalayas' },
            ]}
          />
        </div>
      </div>
      ...
      <section className="med-shell" style={{ position: 'relative', overflow: 'hidden', minHeight: '78vh', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: '1px solid rgba(15,118,110,0.12)' }}>
        <div style={{ position: 'absolute', inset: 0 }}>
          <img className="med-hero-bg" src={PAGE.heroImage || '/Images/services/soundhealing.webp'} alt={PAGE.heroImageAlt || PAGE.h1} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: '58rem', width: '100%', padding: '5rem 1.5rem 4.5rem', textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.8rem', marginBottom: '1.3rem' }}>
            ...
            <span style={{ fontFamily: 'var(--font-inter), sans-serif', fontSize: '0.72rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: '#ffffff', fontWeight: 700 }}>Healing Retreat</span>
            <span style={{ width: 24, height: 1, background: 'rgba(255,255,255,0.6)' }} />
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '2rem' }}>
            {['Deep Rest', 'Forest Immersion', '3–10 Days', 'Small Groups'].map((tag) => (
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            <a href="#inquiry" className="med-cta-btn">Start Your Healing</a>
...
{deepContent.length > 0 && (
  <section id="deep-dive" className="med-shell" style={{ background: '#f7f9f7', padding: '5rem 0', borderBottom: '1px solid rgba(15,118,110,0.08)' }}>
    <div className="med-inner">
      ...
      <h2 className="med-h2" style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
      ...
      {deepContent.map((section, idx) => {
        ...
        return (
          <div key={section.heading} className="med-card" style={{ padding: '2.5rem', marginBottom: idx < deepContent.length - 1 ? '2.5rem' : 0 }}>
            ...
            {section.image && (
              <div className="med-img-wrap" style={{ position: 'relative' }}>
                <img src={section.image} alt={section.imageAlt || section.heading} />
                {idx === 0 && (
                  <div className="med-featured-badge">Featured</div>
                )}
              </div>
            )}
                    ...
                    {listItems.length > 0 && (
                      <ul className="med-list" style={{ margin: '1rem 0' }}>
                        {listItems.map((item, idx) => {
                          ...
                          return (
                            <li key={idx} className="med-list-item" style={{ padding: '0.6rem 0' }}>
      ...
      {/* ── PRIMARY CTA ── */}
      <section id="inquiry" className="med-shell med-section-alt med-healing-cta" style={{ padding: '4rem 0' }}>
        <div className="med-inner">
          <div className="med-eyebrow" style={{ justifyContent: 'center' }}>
            <span className="med-eyebrow-line" />
            <span className="med-eyebrow-text">Ready to Heal</span>
            <span className="med-eyebrow-line" />
          </div>
          <h2 className="med-h2" style={{ textAlign: 'center' }}>Start Your Healing <span>Journey</span></h2>
          <p className="med-body" style={{ textAlign: 'center' }}>
            Find the rest and emotional release you have been needing in the quiet of the Himalayas.
          </p>
          <PrimaryCTA
            label="Start Your Healing"
            vertical="retreat"
            category="healing-retreat"
            sourcePath="/healing-retreat-himalayas"
          />
        </div>
      </section>
      ...
      <nav className="med-shell med-section-alt" style={{ padding: '2.5rem 0', borderTop: '1px solid rgba(15,118,110,0.08)' }}>
        <div className="med-outer">
          <div className="med-nav-grid">
            ...
            <Link href="/retreats/healing-retreat-himalayas" className="med-card" style={{ padding: '0.85rem 1.2rem', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span className="med-body" style={{ margin: 0, fontWeight: 500 }}>Healing Retreats</span>
              <span style={{ color: '#0f766e' }}>→</span>
            </Link>
