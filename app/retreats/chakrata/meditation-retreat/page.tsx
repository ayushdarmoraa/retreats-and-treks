...
export function generateMetadata(): Metadata {
  return {
    title: retreat.title + ' — 3-Day Forest Meditation | Retreats And Treks',
...
export default function Page() {
  ...
  return (
    <TrackedPage page={PATH}>
      <style>{`
        .ckm-inner { max-width: 52rem; margin: 0 auto; padding: 0 2rem; }
        .ckm-wide { max-width: 72rem; margin: 0 auto; padding: 0 2rem; }
        .ckm-eyebrow { display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem; }
        .ckm-eyebrow-line { width: 24px; height: 1px; background: var(--color-primary); }
        .ckm-eyebrow-text { font-family: var(--font-geist-sans), sans-serif; font-size: 0.75rem; letter-spacing: 0.28em; text-transform: uppercase; color: #374151; font-weight: 500; }
        .ckm-section-title { font-family: var(--font-geist-sans), sans-serif; font-size: clamp(1.4rem, 2.5vw, 1.85rem); font-weight: 200; letter-spacing: -0.03em; color: #111; line-height: 1.15; margin: 0 0 2rem; }
        .ckm-section-title span { color: #374151; }
        .ckm-cta-btn { display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.85rem 2.25rem; background: var(--color-primary); color: #fff; text-decoration: none; font-family: var(--font-geist-sans), sans-serif; font-size: 0.78rem; font-weight: 500; letter-spacing: 0.06em; border-radius: 100px; transition: background 0.2s, transform 0.2s; }
        .ckm-cta-btn:hover { background: #0d9e95; transform: translateY(-2px); }
        .ckm-cta-outline { display: inline-flex; align-items: center; gap: 0.4rem; padding: 0.7rem 1.8rem; border: 1px solid rgba(15,118,110,0.3); color: var(--color-primary); text-decoration: none; font-family: var(--font-geist-sans), sans-serif; font-size: 0.75rem; font-weight: 500; letter-spacing: 0.04em; border-radius: 100px; transition: all 0.2s; }
        .ckm-cta-outline:hover { border-color: var(--color-primary); background: rgba(15,118,110,0.04); }
        .ckm-faq-item { border-bottom: 1px solid #eef0ee; padding: 1.5rem 0; }
        .ckm-faq-item:last-child { border-bottom: none; }
      `}</style>

      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(retreatSchema) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }} />

      <Breadcrumb items={[{ name: 'Home', href: '/' }, { name: 'Retreats', href: '/retreats' }, { name: 'Chakrata', href: '/retreats/chakrata' }, { name: 'Meditation Retreat' }]} />
      ...
      <section style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '65vh', textAlign: 'center', paddingTop: '68px' }}>
        <div style={{ position: 'absolute', inset: 0 }}>
          <img src="/Images/experience-hubs/meditation-hero.png" alt="Meditation retreat in the forests of Chakrata, Uttarakhand" fetchPriority="high" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 35%', display: 'block' }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: '44rem', padding: '0 2rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', justifyContent: 'center', marginBottom: '2rem', flexWrap: 'wrap' }}>
            {['Pickup from Dehradun', `${retreat.duration}`, 'All-inclusive', 'Max 12 per group'].map((t) => (
          ...
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
            <a href={wa} className="ckm-cta-btn" style={{ background: '#fff', color: '#0a3d35', fontWeight: 600 }}>Book This Retreat →</a>
            <Link href="/retreats/chakrata" className="ckm-cta-btn" style={{ background: 'transparent', border: '1.5px solid rgba(255,255,255,0.4)' }}>All Chakrata Experiences</Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', background: '#ffffff', padding: '4rem 0' }}>
        <div className="ckm-inner">
          <div className="ckm-eyebrow"><span className="ckm-eyebrow-line" /><span className="ckm-eyebrow-text">Overview</span></div>
          <h2 className="ckm-section-title">What this <span>retreat offers</span></h2>
      ...
      <section style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', background: '#f7f9f7', padding: '4rem 0' }}>
        <div className="ckm-inner">
          <div className="ckm-eyebrow"><span className="ckm-eyebrow-line" /><span className="ckm-eyebrow-text">Daily Schedule</span></div>
          <h2 className="ckm-section-title">Your <span>itinerary</span></h2>
      ...
      <section style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', background: '#0a1f1c', padding: '3.5rem 0', textAlign: 'center' }}>
        <div style={{ maxWidth: '44rem', margin: '0 auto', padding: '0 2rem' }}>
          <h3 style={{ fontFamily: 'var(--font-geist-sans), sans-serif', fontSize: 'clamp(1.1rem, 2vw, 1.4rem)', fontWeight: 200, color: '#fff', margin: '0 0 0.6rem' }}>Ready to begin?</h3>
          <p style={{ fontFamily: 'var(--font-geist-sans), sans-serif', fontSize: '0.82rem', color: 'rgba(255,255,255,0.5)', fontWeight: 300, margin: '0 0 1.5rem', lineHeight: 1.7 }}>Ask your questions, check availability, or book directly — we reply within hours.</p>
          <a href={wa} className="ckm-cta-btn">WhatsApp Us →</a>
        </div>
      </section>
      ...
      <section style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', background: '#ffffff', padding: '4rem 0' }}>
        <div className="ckm-inner">
          <div className="ckm-eyebrow"><span className="ckm-eyebrow-line" /><span className="ckm-eyebrow-text">What&apos;s Included</span></div>
      ...
      <section style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', background: '#f7f9f7', padding: '4rem 0' }}>
        <div className="ckm-inner">
          <div className="ckm-eyebrow"><span className="ckm-eyebrow-line" /><span className="ckm-eyebrow-text">Questions</span></div>
          <h2 className="ckm-section-title">Frequently asked <span>questions</span></h2>
          <div>
            {retreat.faqs.map((faq, i) => (
              <div key={i} className="ckm-faq-item">
      ...
      <section style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', background: '#ffffff', padding: '4rem 0' }}>
        <div className="ckm-inner">
          <div className="ckm-eyebrow"><span className="ckm-eyebrow-line" /><span className="ckm-eyebrow-text">Also in Chakrata</span></div>
          <h2 className="ckm-section-title">Explore more <span>Chakrata experiences</span></h2>
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <Link href="/retreats/chakrata" className="ckm-cta-outline">All Chakrata Retreats & Treks →</Link>
            <Link href="/retreats/chakrata/yoga-retreat" className="ckm-cta-outline">Yoga Retreat →</Link>
            <Link href="/retreats/chakrata/weekend-retreat-from-dehradun" className="ckm-cta-outline">Weekend Retreat →</Link>
            <Link href="/treks/location/chakrata" className="ckm-cta-outline">Chakrata Treks →</Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', position: 'relative', overflow: 'hidden', minHeight: '40vh', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: '44rem', padding: '3rem 2rem' }}>
          <h2 style={{ fontFamily: 'var(--font-geist-sans), sans-serif', fontSize: 'clamp(1.3rem, 2.5vw, 1.8rem)', fontWeight: 200, color: '#fff', margin: '0 0 0.75rem' }}>Begin Your Meditation Journey</h2>
          <p style={{ fontFamily: 'var(--font-geist-sans), sans-serif', fontSize: '0.85rem', color: 'rgba(255,255,255,0.55)', fontWeight: 300, lineHeight: 1.75, margin: '0 0 1.5rem' }}>Forest silence at 2,200m. Guided practice. Small groups. Tell us when you&apos;d like to come.</p>
          <a href={wa} className="ckm-cta-btn" style={{ fontSize: '0.85rem', padding: '1rem 2.5rem' }}>Book This Retreat →</a>
        </div>
      </section>
    </TrackedPage>
  );
